...
const { buildObjectiveAssessment } = require('../../common/questionAssessment');
...
module.exports.readingFillInTheBlanksResult = asyncWrapper(async (req, res) => {
    ...
    res.status(200).json({
        ...
        assessment: buildObjectiveAssessment({
            questionType: 'reading',
            subtype: 'reading_fill_in_the_blanks',
            score: result?.result?.score,
            maxScore: result?.result?.totalBlanks,
            feedback: result.feedback,
            meta: {
                totalBlanks: result?.result?.totalBlanks,
            },
        }),
    });
});
