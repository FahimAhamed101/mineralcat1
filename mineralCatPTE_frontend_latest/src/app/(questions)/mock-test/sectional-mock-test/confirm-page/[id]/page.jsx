...
const getAttemptStorageKey = (mockTestId) =>
  `sectional-mock-attempt:${mockTestId}`;
...
const SummarizeWrittenTextComponent = ({ question, onAnswer, answerValue }) => {
  const [summary, setSummary] = useState(() => normalizeTextAnswer(answerValue));
...
const WriteEmailComponent = ({ question, onAnswer, answerValue }) => {
  ...
  const submitAnswer = useCallback(async (questionId, answer) => {
    ...
    try {
      ...
      if (answer instanceof Blob) {
        ...
        response = await fetchWithAuth(`${baseUrl}/sectional-mock-test/result-single-question`, {
          method: "POST",
          body: formData,
        });
      } else {
...
}, [testData, currentQuestionIndex, isSubmitting, queueAnswerSubmission]);
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Uses {testData.aiCreditCost ?? 1} AI credits once per attempt. Answers save in the background while you move to the next question.
              </p>
            </div>
  ...
  {isSubmitting ? (
    ...
    isLastQuestion ? 'Submit Test' : 'Next'
      ...
      <MockScoreReportModal
        ...
        testMode="Sectional Mock Test"
        testId={mockTestId}
        testDuration={testData?.duration}
        questionCount={testData?.questions?.length}
        sectionType={testData?.type}
      />
