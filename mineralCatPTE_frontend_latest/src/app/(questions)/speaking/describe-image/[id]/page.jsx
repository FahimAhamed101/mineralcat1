...
function toDisplayOutOf90FromTrait(value, max = TRAIT_SCALE_MAX) {
  ...
  return Math.round((numericValue / max) * DISPLAY_SCORE_MAX);
}
...
export default function DescribeImageQuestionPage({ params }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => setTimeLeft((time) => time - 1), 1000);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetchWithAuth(
        `${baseUrl}/test/speaking/describe_image/result`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const assessment = result ? getQuestionAssessment(result, "describe_image") : null;
  ...
  return (
    <div className="w-full lg:w-full lg:max-w-[80%] mx-auto py-6 px-2 relative">
      ...
      <p className="text-gray-700 mb-6">
        Look at the image and describe it clearly. You will have {RECORD_SECONDS}
      ...
      <div className="border border-[#810000] rounded p-4 mb-4 bg-[#faf9f9]">
        {question.imageUrl ? (
          <img
            src={question.imageUrl}
            alt={question.heading || "Describe Image"}
            className="mx-auto max-h-[460px] w-full object-contain rounded bg-white"
          />
        ) : (
          <p className="text-sm font-medium text-gray-500 text-center">
            Image is missing for this question.
          </p>
        )}
      </div>
      ...
      <div className="border border-[#810000] rounded-lg bg-[#faf9f9] p-5 mb-6">
        ...
        <div className="flex items-center justify-center gap-3 flex-wrap">
          <button
            ...
            disabled={isRecording || submitLoading}
          >
            Restart
          </button>
          <button
            ...
            disabled={isRecording || timeLeft === 0 || submitLoading}
          >
            Start
          </button>
          <button
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          ...
          >
            <div className="bg-gradient-to-r from-[#7D0000] to-[#c0392b] px-6 py-4 flex items-center justify-between">
              <div>
                <h2 className="text-white text-xl font-bold">Describe Image - AI Score</h2>
                <p className="text-white/75 text-sm mt-0.5">Image description scoring</p>
              </div>
              <button
            ...
            {result?.success ? (
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-4 bg-gray-50 rounded-xl p-4">
                    <div className="flex flex-col items-center gap-1 bg-white rounded-lg p-3 shadow-sm">
                      <ScoreGauge
                        value={toDisplayOutOf90FromTrait((contentScore + pronunciationScore + fluencyScore) / 3, traitScaleMax || TRAIT_SCALE_MAX)}
                        label="Speaking"
                        color="#810000"
                        footer="Out of 90"
                      />
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-blue-50 rounded-xl p-3 text-center">
                      <p className="text-xl font-black text-blue-700">
                        {toDisplayOutOf90FromTrait(contentScore, traitScaleMax || TRAIT_SCALE_MAX)}
                ...
                <div className="bg-[#fffbea] border border-yellow-200 rounded-xl p-4 text-sm text-yellow-800">
                  ...
                  <p>Speaking uses the average of Content, Pronunciation, and Fluency.</p>
                  <p className="mt-2">
                    Total trait score: {formatScore(totalTraitScore)} / {((traitScaleMax || TRAIT_SCALE_MAX) * 3).toFixed(0)}
                  </p>
                </div>
              </div>
            ) : (
