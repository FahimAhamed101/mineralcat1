...
export default function ResponseSitutation() {
  ...
  // Load selectedIds from global key if mockTest
  useEffect(() => {
    if (isMockTest) {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(
        `${baseUrl}/test/speaking/respond-to-a-situation?page=${currentPage}&limit=${itemsPerPage}`
      );
      const data = await response.json();
      if (data?.questions) {
        setTotalPages(Math.ceil((data.questionsCount) / itemsPerPage));
  ...
  const handleAddMockQuestions = () => {
    location?.state?.url ? navigate(location.state.url) : navigate("/mock/full/add");
  };
      ...
      <div className="bg-red-700 text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-medium">Speaking (Respond to a Situation)</h1>
          </button>
        </div>
        {isMockTest ? (
          <Button
